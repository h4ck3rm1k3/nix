...
if [ -f "$(dirname $OUTPUT)/rascunho1" ] ; then
   echo "Já existe um arquivo chamado \"$(dirname $OUTPUT)/rascunho1\""
   exit 1
fi

if [ -f "$(dirname $OUTPUT)/rascunho2" ] ; then
   echo "Já existe um arquivo chamado \"$(dirname $OUTPUT)/rascunho2\""
   exit 1
fi
...
' "$1" >> "$(dirname $OUTPUT)/rascunho1"

sed -i -e 's/^\(.*\)$/ \1/g;' "$(dirname $OUTPUT)/rascunho1"


# cat nameSpace.txt | cut -d' ' -f2-3 | sed 's/\//\\\//g; s/\([^ ]*\) <\([^>]*\)>/   s\/<\2\\([^>]*\\)>\/\1\\1\/g\;/g;'

if grep -q " rdf:" "$(dirname $OUTPUT)/rascunho1" ; then
   echo "@prefix rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> ." >> "$(dirname $OUTPUT)/rascunho2";
fi

if grep -q " rdfs:" "$(dirname $OUTPUT)/rascunho1" ; then
   echo "@prefix rdfs: <http://www.w3.org/2000/01/rdf-schema#> ." >> "$(dirname $OUTPUT)/rascunho2";
fi

if grep -q " dbc:" "$(dirname $OUTPUT)/rascunho1" ; then
   echo "@prefix dbc: <http://dbpedia.org/resource/Category:> ." >> "$(dirname $OUTPUT)/rascunho2";
fi

if grep -q " dbo:" "$(dirname $OUTPUT)/rascunho1" ; then
   echo "@prefix dbo: <http://dbpedia.org/ontology/> ." >> "$(dirname $OUTPUT)/rascunho2";
fi

if grep -q " dbp:" "$(dirname $OUTPUT)/rascunho1" ; then
   echo "@prefix dbp: <http://dbpedia.org/property/> ." >> "$(dirname $OUTPUT)/rascunho2";
fi

if grep -q " dbr:" "$(dirname $OUTPUT)/rascunho1" ; then
   echo "@prefix dbr: <http://dbpedia.org/resource/> ." >> "$(dirname $OUTPUT)/rascunho2";
fi

if grep -q " dc:" "$(dirname $OUTPUT)/rascunho1" ; then
   echo "@prefix dc: <http://purl.org/dc/elements/1.1/> ." >> "$(dirname $OUTPUT)/rascunho2";
fi

if grep -q " foaf:" "$(dirname $OUTPUT)/rascunho1" ; then
   echo "@prefix foaf: <http://xmlns.com/foaf/0.1/> ." >> "$(dirname $OUTPUT)/rascunho2";
fi

if grep -q " owl:" "$(dirname $OUTPUT)/rascunho1" ; then
   echo "@prefix owl: <http://www.w3.org/2002/07/owl#> ." >> "$(dirname $OUTPUT)/rascunho2";
fi

if grep -q "\^\^xsd:" "$(dirname $OUTPUT)/rascunho1" || grep -q " xsd:" "$(dirname $OUTPUT)/rascunho1" ; then
   echo "@prefix xsd: <http://www.w3.org/2001/XMLSchema#> ." >> "$(dirname $OUTPUT)/rascunho2";
fi

if grep -q " obo:" "$(dirname $OUTPUT)/rascunho1" ; then
   echo "@prefix obo: <http://www.geneontology.org/formats/oboInOwl#> ." >> "$(dirname $OUTPUT)/rascunho2";
fi
...
sed -i -e 's/^ \(.*\)$/\1/' "$(dirname $OUTPUT)/rascunho1"
cat "$(dirname $OUTPUT)/rascunho2" "$(dirname $OUTPUT)/rascunho1" > "$OUTPUT"

rm "$(dirname $OUTPUT)/rascunho1"
rm "$(dirname $OUTPUT)/rascunho2"
